...
public interface IRitual extends IForgeRegistryEntry<IRitual> {
	...
	/**
	 * The input items that trigger the ritual. Add an ingredient twice for it to be
	 * counted twice
	 *
	...
	/**
	 * @return the raw output without modifications. This is mainly used for showing
	 *         the recipe in JEI
	 */
	...
	/**
	 * @return the time, in ticks, of how long this ritual should last. Negative
	 *         values make it go indefinitely
	 */
	public int getTime();
	...
	/**
	 * Ideally you should use CovensAPI#getCirclesIntegerForRitual(...) to get human
	 * readable code. You should either cache this value or, even better, return a
	 * hard coded value for even better performance. The bit encoding is an 8 bit
	 * number BBMMSSNN where the first 6 bits are, 2 by 2, the metadata of the
	 * circle type from bigger to smaller, and the last 2 the number of total
	 * circles.
	 *
	...
	/**
	 * The power that should be drained each tick for this to work. If the power is
	 * not available,
	 * {@link #onLowPower(EntityPlayer, TileEntity, World, BlockPos, NBTTagCompound, int)}
	...
	/**
	 * The power that should be drained at the beginning of the ritual for it to
	 * trigger at all. If there is not enough power the ritual is not started at all
	 * and the stacks on the ground are not consumed
	 *
	 * @return the value in AP/ME/MP
	 */
