package com.covens.client.render.entity.layer;

import org.lwjgl.opengl.GL11;

import com.covens.client.render.entity.model.ModelMantle;
...
public class MantleLayer implements LayerRenderer<AbstractClientPlayer> {
	...
	public MantleLayer(ModelPlayer playerModel) {
		this.model = new ModelMantle(playerModel);
	}
	...
	public void doRenderLayer(AbstractClientPlayer player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		if (BaublesApi.getBaublesHandler(player).getStackInSlot(BaubleType.BODY.getValidSlots()[0]).getItem() == ModItems.mantle) {
			...
			this.model.render(player, player.limbSwing, player.limbSwingAmount, ageInTicks + partialTicks, player.rotationYaw, player.rotationPitch, scale);
			GL11.glPopMatrix();
		}
	}
	...
	public boolean shouldCombineTextures() {
		return false; // I have no clue what this means, try and set it to true if something about the
						// texture is weird
	}
