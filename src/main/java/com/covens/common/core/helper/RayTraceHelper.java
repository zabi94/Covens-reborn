...
public final class RayTraceHelper {
	...
	public static RayTraceResult rayTracePlayerSight(EntityPlayer source, double range, boolean includeEntities) {
		...
		double d1 = source.posY + source.eyeHeight;
		double d2 = source.posZ;
		World world = source.world;
		Vec3d vec3d = new Vec3d(d0, d1, d2);
		Vec3d lookVector = source.getLookVec().scale(range);
		Vec3d forward = vec3d.add(lookVector);
		...
		if (includeEntities) {
			...
			Entity entity = null;
			List<Entity> list = world.getEntitiesWithinAABBExcludingEntity(source, source.getEntityBoundingBox().expand(lookVector.x, lookVector.y, lookVector.z).grow(1.0D));
			double d6 = 0.0D;
			for (Entity ent : list) {
