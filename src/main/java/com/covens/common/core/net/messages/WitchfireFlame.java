...
public class WitchfireFlame extends SimpleMessage<WitchfireFlame> {
	...
	public IMessage handleMessage(MessageContext context) {
		for (int i = 0; i < 100; i++) {
			...
			Covens.proxy.spawnParticle(ParticleF.COLORED_FLAME, posA.getX() + r.nextDouble(), posA.getY() + r.nextDouble() * 2, posA.getZ() + r.nextDouble(), 0.05 * r.nextGaussian(), 0.1 * r.nextGaussian(), 0.05 * r.nextGaussian(), color);
			Covens.proxy.spawnParticle(ParticleF.COLORED_FLAME, posB.getX() + r.nextDouble(), posB.getY() + r.nextDouble() * 2, posB.getZ() + r.nextDouble(), 0.05 * r.nextGaussian(), 0.1 * r.nextGaussian(), 0.05 * r.nextGaussian(), color);
		}
