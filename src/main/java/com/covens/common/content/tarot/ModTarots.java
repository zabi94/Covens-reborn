...
public class ModTarots {
	...
	public static void init() {
		...
		TarotHandler.registerTarot(new QuickTarot("nitwit", p -> (p.getCapability(MPContainer.CAPABILITY, null).getMaxAmount() / 800) > 0, null, p -> p.getCapability(MPContainer.CAPABILITY, null).getMaxAmount() / 800));
		...
		TarotHandler.registerTarot(new QuickTarot("silver_sword", p -> p.getCapability(CapabilityTransformation.CAPABILITY, null).getType() == DefaultTransformations.HUNTER || TarotHandler.isPlayerProtected(p), p -> p.getCapability(CapabilityTransformation.CAPABILITY, null).getType() != DefaultTransformations.HUNTER, p -> p.getCapability(CapabilityTransformation.CAPABILITY, null).getLevel()));
