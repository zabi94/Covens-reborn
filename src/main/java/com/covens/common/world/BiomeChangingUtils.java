package com.covens.common.world;
...
public class BiomeChangingUtils {
	...
	public static void changeWholeBiome(BlockPos pos, Biome newBiome, World world) {
		...
		for (int i = 0; i < 255; i++) {
			setArrayValue(array, id, i);
		}
		ch.markDirty();
		refreshChunk(ch);
	}

	// This is used to proxy the array type, as it might be modified to an int array
	// by mods like JeID
	...
	public static class BiomeChangerWalker {
		...
		/**
		 * Use this class to efficiently change biome to multiple block coords spanning
		 * across multiple chunks. <br>
		 * <br>
		 * Use {@link #visit(Chunk, BlockPos)} or {@link #visit(World, BlockPos)} to add
		 * blocks to change, using MutableBlockPos for efficiency when too many. <br>
		 * <br>
		 * When done adding coords use {@link #complete()} to complete the process.
		 * Don't leave a walker that has visited at least a coordinate hanging, as it
		 * may cause desyncs and random weirdness. <br>
		 * <br>
		 * Biomes with the BiomeDict tag IMMUTABLE won't be converted unless using the
		 * {@link #visitImmutable(Chunk, BlockPos)} and
		 * {@link #visitImmutable(World, BlockPos)} methods.
		 *
		 * @param destinationBiome What biome to convert blocks to
		 * @see CovensAPI#IMMUTABLE
		 */
		...
		/**
		 * Changes a biome column in the chunk. Won't change biomes that have the
		 * IMMUTABLE dict type
		 *
		 ...
		 * @throws IllegalStateException if invoked after being marked as
		 *                               {@link #complete()}
		 * @see CovensAPI#IMMUTABLE
		 */
		public boolean visit(Chunk ch, BlockPos pos) {
			if (BiomeDictionary.hasType(ch.getWorld().getBiome(pos), CovensAPI.getAPI().IMMUTABLE)) {
				return false;
			}
		...
		/**
		 * Changes a biome column in the world. Won't change biomes that have the
		 * IMMUTABLE dict type
		 *
		...
		public boolean visit(World world, BlockPos pos) {
			return this.visit(world.getChunk(pos), pos);
		}

		/**
		 * Changes a biome column in the chunk. Will also change biomes that have the
		 * IMMUTABLE dict type
		 *
		 * @param ch  The chunk the column belongs to
		 * @param pos The position of the column. Y value is ignored
		...
		public boolean visitImmutable(Chunk ch, BlockPos pos) {
			...
			return setArrayValue(ch.getBiomeArray(), this.biomeId, (pos.getX() - (ch.x * 16)) + (16 * (pos.getZ() - (ch.z * 16))));
		}

		/**
		 * Changes a biome column in the world. Will also change biomes that have the
		 * IMMUTABLE dict type
		 *
		 * @param world The world the column belongs to
		 * @param pos   The position of the column. Y value is ignored
		...
		public boolean visitImmutable(World world, BlockPos pos) {
			return this.visitImmutable(world.getChunk(pos), pos);
		}

		/**
		 * This method will mark all the chunks interested by the instance as dirty and
		 * will synchronize them to the clients in the dimension. Any following
		 * invocation to other methods from this class will likely result in an
		 * exception. <br>
		 * <br>
		 * Don't forget to call this if this walker has used any visit or visitImmutable
		 * methods
		 *
